...
# 此版本去除了word2vec向量的语义评估模型
class Model_Engine(object):
    def __init__(self,tokenizer, hnsw_model_dir=None, hnsw_dim = 256):
        ...
        if os.path.isdir(hnsw_model_dir) and len(os.listdir(hnsw_model_dir)):
            ...
            for file_name in file_name_list:
                if file_name.endswith(".bin"):
                    self.hnsw_index[file_name.replace(".bin","")] = self.load_hnsw_index(os.path.join(hnsw_model_dir,file_name))
                    # logging.info("加载HNSW召回模型:{}".format(os.path.join(hnsw_model_dir,file_name)))
                    # current_app.logger.info("加载HNSW召回模型:{}".format(os.path.join(hnsw_model_dir,file_name)))
                elif file_name.endswith(".pkl"):
                    self.id_sentence_intent_map[file_name.replace(".pkl","")] = self.json_deserialization(os.path.join(hnsw_model_dir,file_name))
                    # logging.info("加载HNSW召回模型映射关系:{}".format(os.path.join(hnsw_model_dir,file_name)))
            # logging.info("召回字典：{}".format(self.hnsw_index))
    ...
    def hnsw_index_update(self, from_json, to_dir, to_file):
        ...
        # current_app.logger.info("更新HNSW召回模型:{},更新HNSW召回模型映射关系:{}".format(os.path.join(to_dir,to_file+".bin"),os.path.join(to_dir,to_file+".pkl")))

    # 召回模型org删除接口
    # org_dir：规则保存目录
    # org_name：org名称，例如：开场白_yB7b2E
    # 删除 org_dir/org_name.pkl
    def hnsw_index_delete(self,org_dir,org_name):
        obj_bin = os.path.join(org_dir,org_name + ".bin")
        obj_pkl = os.path.join(org_dir,org_name + ".pkl")
        if os.path.exists(obj_bin):
            os.remove(obj_bin) # 删除召回模型指定org的bin文件
            self.hnsw_index.pop(org_name) # 删除召回模型指定org字典值
        if os.path.exists(obj_pkl): # 删除召回模型指定org的pkl文件
            os.remove(obj_pkl) # 删除召回模型指定org字典值
        current_app.logger.info("删除Bin模型:{},向量映射关系:{}".format(obj_bin,obj_pkl))
    ...
    def build_hnsw_index(self, from_json, to_file):
        ...
        id_sentence_dict = {}
        id_embedding_dict = {}
        index = hnswlib.Index(space = 'cosine', dim = self.hnsw_dim) # dim = 512
        index.init_index(max_elements=60000, ef_construction=200, M=32)
        for label,sentence_list in from_json.items():
            for sentence in sentence_list:
                # 本地加载并编码
                # embedding = self.model.encode(sentence,show_progress_bar=False)
                input_ids = self.convert_sentence_to_features(sentence.strip())
                embedding_tmp = requests.post("http://127.0.0.1:8501/v1/models/model_cls_slot:predict",data=json.dumps({"instances": [{"ori_input_quests": input_ids}]}))
                embedding_res = embedding_tmp.json()['predictions'][0]
                # 用于召回
                cls_feature = np.array(embedding_res['cls_feature'])
                # 用于计算相似度
                cls_seq_emb = np.array(embedding_res['cls_seq_emb'])
                sentence_intents.append(id_counter)
                sentence_vectors.append(cls_feature)
                id_intent_dict[id_counter] = label
                id_sentence_dict[id_counter] = sentence
                id_embedding_dict[id_counter] = cls_seq_emb
                id_counter += 1
        ...
        current_app.logger.info("更新图向量文件:{},更新图向量条数:{}".format(to_file,id_counter))
        return index
    ...
    def search(self, parent_node,query, k):
        ...
        # 本地加载并编码
        input_ids = self.convert_sentence_to_features(query.strip())
        query_embedding_tmp = requests.post("http://127.0.0.1:8501/v1/models/model_cls_slot:predict",data=json.dumps({"instances": [{"ori_input_quests": input_ids}]}))
        query_embedding = np.array(query_embedding_tmp.json()['predictions'][0]['cls_feature'])
    ...
    # 通过父亲节点以及query从对应的hnsw模型中召回数据
    def models_parse_v1(self, parent_node, query, filter_score = 0.7, topN = 5):
        id_intent_dict = self.id_sentence_intent_map[parent_node]["id_intent_dict"]
        id_sentence_dict = self.id_sentence_intent_map[parent_node]["id_sentence_dict"]
        id_embedding_dict = self.id_sentence_intent_map[parent_node]["id_embedding_dict"]
        # 本地加载并编码
        input_ids = self.convert_sentence_to_features(query.strip())
        query_embedding_res = requests.post("http://127.0.0.1:8501/v1/models/model_cls_slot:predict",data=json.dumps({"instances": [{"ori_input_quests": input_ids}]}))
        query_embedding = np.array(query_embedding_res.json()['predictions'][0]['cls_feature'])

        labels, distances = self.hnsw_index[parent_node].knn_query(query_embedding, k = topN)
        ResDictListTmp = []
        for index in range(topN):
            ...
            # distance = distances[0][index]
            embedding = id_embedding_dict[labels[0][index]]
            diff_score = self.difflib_sim(query, sentence)
            sbert_score = self.cosine_sim(query_embedding, embedding)
            ResDictListTmp.append({'query':query, 'retrieved':sentence, 'intent':intent, 'diff_score':diff_score, 'sbert_score':sbert_score})

        FineSortList = []
        FineSortSbert = sorted(ResDictListTmp, key=lambda x:x['sbert_score'], reverse = True)[0]
        if float(FineSortSbert['sbert_score']) >= filter_score:
        ...
        FineSortDiff = sorted(ResDictListTmp, key=lambda x:x['diff_score'], reverse = True)[0]
        if float(FineSortDiff['diff_score']) >= filter_score:
        ...
        # current_app.logger.info("请求节点:{},请求文本:{},响应结果:{}".format(parent_node,query,FineSortList))
        return FineSortList

    # 通过父亲节点以及query从对应的hnsw模型中召回数据
    # 版本说明:
    # 1.移除word2vec计算文本相似度功能
    # 2.添加各个得分值得加权平均值作为融合得分
    def models_parse_v2(self, parent_node, query, filter_score = 0.7, topN = 5):
        id_intent_dict = self.id_sentence_intent_map[parent_node]["id_intent_dict"]
        id_sentence_dict = self.id_sentence_intent_map[parent_node]["id_sentence_dict"]
        id_embedding_dict = self.id_sentence_intent_map[parent_node]["id_embedding_dict"]
        # 本地加载并编码
        input_ids = self.convert_sentence_to_features(query.strip())
        query_embedding_tmp = requests.post("http://127.0.0.1:8501/v1/models/model_cls_slot:predict",data=json.dumps({"instances": [{"ori_input_quests": input_ids}]}))
        query_embedding_res = query_embedding_tmp.json()['predictions'][0]
        # 用于召回
        query_cls_feature = query_embedding_res['cls_feature']
        # 用于计算相似度
        query_cls_seq_emb = query_embedding_res['cls_seq_emb']
        
        labels, distances = self.hnsw_index[parent_node].knn_query(query_cls_feature, k = topN)
        ResDictListTmp = []
        for index in range(topN):
            intent = id_intent_dict[labels[0][index]]
            sentence = id_sentence_dict[labels[0][index]]
            ...
            embedding = id_embedding_dict[labels[0][index]]
            diff_score = self.difflib_sim(query, sentence)
            sbert_score = self.cosine_sim(query_cls_seq_emb, embedding)
            avg_score = np.mean([diff_score, sbert_score])
            ResDictListTmp.append({'query':query, 'retrieved':sentence, 'intent':intent, 'diff_score':diff_score, 'sbert_score':sbert_score,'avg_score':avg_score})

        # current_app.logger.info("请求节点:{},请求文本:{},响应结果:{}".format(parent_node,query,FineSortList))
        FineSortAvg = sorted(ResDictListTmp, key=lambda x:x['avg_score'], reverse = True)[0]
        if float(FineSortAvg['avg_score']) >= filter_score:
            return {"match_score":FineSortAvg['avg_score'],"match_node":FineSortAvg['intent'],"match_rule":FineSortAvg['retrieved'],"match_module":"avg_score"}
        else:
            return {}
        
    # 计算语句向量的相似度
    def cosine_sim(self, nd_a, nd_b):
        nd_a = np.array(nd_a)
        nd_b = np.array(nd_b)
        return np.sum(nd_a * nd_b) / (np.sqrt(np.sum(nd_a**2)) * np.sqrt(np.sum(nd_b**2)))

    # 计算语句的编辑距离
    def difflib_sim(self, str_a , str_b):
    ...
    # 将中文文本id化用于请求bert模型
    def convert_sentence_to_features(self,sentence,max_len = 32):
        tokens = []
        tokens.append("[CLS]")
