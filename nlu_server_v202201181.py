...
#     logging.info("######{}######".format(RoughRecall_DictList.keys()))
...
def Recall_Sort_V1(parent_node_name,query,topN = 10):
    # query问题粗召回
    RoughRecall_DictList = hnsw_engine.recall_torchserve(parent_node_name, query, topN)
    # logging.info("######{}######".format(type(RoughRecall_DictList)))
    sim = {}
    RecallSort = []
    for row in RoughRecall_DictList:
        # logging.info("*****{}*****".format(type(row['query_embedding'])))
        sim['query'] = row['query']
        sim['retrieved'] = row['recall_sentences']
        sim['intent'] = row['recall_intents']
        sim['w2v_cos'] = cos_sim(row['query_word2vec'],row['recall_word2vec'])
        sim['diff_score'] = Difflib_SeqMatcher(row['query'],row['recall_sentences'])
        sim['sbert_score'] = cos_sim(row['query_embedding'], row['recall_embedding'])
        RecallSort.append(sim)
    
    # {'query','retrieved','intent','lcs_score','edit_dist','diff_score','jaccard','w2v_cos','w2v_eucl','w2v_pearson','fast_cos','fast_eucl','fast_pearson','sbert_score'}
    return RecallSort

def FineSortFilter(dict_data, filter_threshold = 0.99):
...
def Auto_Sort_Keyword_Semantic_V11(parent_node, query, model_threshold = 0.99):
    ...
    FineSortTmp = list(filter(FineSortFilter, FineSortList))
...
def search():
    if request.method == 'POST':
        ...
        try:
            ...
            logging.info("pid:{},org:{},query:{},predict_intent:{}".format(pid,parent_node_name,query,sem_res))
        except:
...
if __name__ == '__main__':
    # start = time.time()
    # sem_res,co_sem_res = Auto_Sort_Keyword_Semantic_V11('开场白', '我不要')
    # end = time.time()
    ...
    # ./wrk -t 5 -c 50 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    ...
    # gunicorn -c gunicorn.py nlu_server_v202201181:app --preload
    # ps ax | grep gunicorn
    ...
    # kernprof -l -v nlu_server_v202201181.py >kernprof.log
    ...
    #   Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    17.04ms    5.05ms 219.30ms   99.32%
    #     Req/Sec    58.89      6.64    70.00     61.74%
    #   Latency Distribution
    #      50%   16.14ms
    #      75%   16.27ms
    #      90%   19.20ms
    #      99%   21.92ms
    #   1759 requests in 30.03s, 3.22MB read
    # Requests/sec:     58.57
    ...
    # 5 threads and 50 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    94.26ms   62.58ms 452.12ms   75.68%
    #     Req/Sec    32.44     13.25    70.00     72.41%
    # Latency Distribution
    #     50%   78.35ms
    #     75%  120.19ms
    #     90%  177.93ms
    #     99%  302.73ms
    # 954 requests in 30.04s, 1.75MB read
    # Requests/sec:     31.76
    # Transfer/sec:     59.55KB

    # ./wrk -t 5 -c 100 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    # 5 threads and 100 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    16.62ms    9.56ms 418.17ms   99.89%
    #     Req/Sec    60.81      6.99    70.00     64.86%
    # Latency Distribution
    #     50%   15.94ms
    #     75%   15.97ms
    #     90%   19.18ms
    #     99%   21.30ms
    # 1804 requests in 30.03s, 3.30MB read
    # Requests/sec:     60.07
    # Transfer/sec:    112.62KB

    # ./wrk -t 5 -c 200 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    # 5 threads and 200 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    17.43ms   55.62ms   2.32s    99.94%
    #     Req/Sec    61.92      5.60    70.00     76.90%
    # Latency Distribution
    #     50%   16.06ms
    #     75%   16.08ms
    #     90%   16.11ms
    #     99%   16.75ms
    # 1720 requests in 30.03s, 3.15MB read
    # Requests/sec:     57.27
    # Transfer/sec:    107.37KB

    # ./wrk -t 5 -c 400 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    # 5 threads and 400 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    16.33ms    0.99ms  19.56ms   89.38%
    #     Req/Sec    61.17      5.48    70.00     68.67%
    # Latency Distribution
    #     50%   15.98ms
    #     75%   16.02ms
    #     90%   19.12ms
    #     99%   19.48ms
    # 1836 requests in 30.05s, 3.36MB read
    # Socket errors: connect 0, read 8, write 0, timeout 0
    # Requests/sec:     61.10
    # Transfer/sec:    114.56KB

    # ./wrk -t 5 -c 800 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    # 5 threads and 800 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    17.15ms   26.52ms   1.12s    99.89%
    #     Req/Sec    60.31      7.57    70.00     66.09%
    # Latency Distribution
    #     50%   15.99ms
    #     75%   16.03ms
    #     90%   19.11ms
    #     99%   21.84ms
    # 1747 requests in 30.06s, 3.20MB read
    # Socket errors: connect 0, read 128, write 0, timeout 0
    # Requests/sec:     58.12
    # Transfer/sec:    108.97KB

    # ./wrk -t 10 -c 1000 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    # 10 threads and 1000 connections
    # Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    18.38ms   54.42ms   2.22s    99.94%
    #     Req/Sec    57.77      9.67    70.00     81.56%
    # Latency Distribution
    #     50%   16.08ms
    #     75%   18.89ms
    #     90%   19.04ms
    #     99%   20.96ms
    # 1638 requests in 30.10s, 3.00MB read
    # Socket errors: connect 0, read 64, write 0, timeout 2
    # Requests/sec:     54.42
    # Transfer/sec:    102.04KB

    # ./wrk -t 1 -c 1000 -d 30s --latency --timeout 10s -s post.lua http://localhost:1472/nlu/search
    # Running 30s test @ http://localhost:1472/nlu/search
    #   20 threads and 1000 connections
    #   Thread Stats   Avg      Stdev     Max   +/- Stdev
    #     Latency    17.05ms    1.56ms  21.63ms   66.67%
    #     Req/Sec    58.60      6.74    70.00     53.82%
    #   Latency Distribution
    #      50%   15.98ms
    #      75%   19.07ms
    #      90%   19.14ms
    #      99%   20.80ms
    #   1764 requests in 30.08s, 3.23MB read
    #   Socket errors: connect 0, read 4, write 0, timeout 0
    # Requests/sec:     58.65
    # Transfer/sec:    109.97KB
    ...
    # ab -n200 -c4 -T application/json -p post.json  http://127.0.0.1:1472/nlu/search
