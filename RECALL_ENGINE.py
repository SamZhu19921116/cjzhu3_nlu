...
from nlu_server_v202202171 import app

# logging.basicConfig(format="%(levelname)s - %(asctime)s: %(message)s", datefmt="%H:%M:%S", level=logging.INFO)
logging = logging.getLogger(__name__)
from gensim.models import Word2Vec,KeyedVectors
class Recall_Engine(object):
    def __init__(self,tokenizer,wor2vec_model,jieba_model,hnsw_model_dir=None,hnsw_dim = 256):
        ...
        if os.path.isdir(hnsw_model_dir) and len(os.listdir(hnsw_model_dir)):
            ...
            for file_name in file_name_list:
                if file_name.endswith(".bin"):
                    ...
                    # current_app.logger.info("加载HNSW召回模型映射关系:{}".format(os.path.join(hnsw_model_dir,file_name)))
            logging.info("召回字典：{}".format(self.hnsw_index))
            # current_app.logger.info("召回字典：{}".format(self.hnsw_index))
    ...
    # 计算语句向量的相似度
    def cosine_similar_v1(self, nd_a, nd_b):
        nd_a = np.array(nd_a)
        nd_b = np.array(nd_b)
        return np.sum(nd_a * nd_b) / (np.sqrt(np.sum(nd_a**2)) * np.sqrt(np.sum(nd_b**2)))

    # 计算两个句子的余弦相似度
    def cosine_similar_v2(self,sen_a_vec, sen_b_vec):
