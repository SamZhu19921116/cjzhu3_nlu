...
rules.add_rule("rule_zhongan_material","*{disclosure_verb:disclosure_verb}*{disclosure_noun:disclosure_noun}*")
rules.add_rule("rule_zhongan_material","*{disclosure_verb0:disclosure_verb0}*")
# 征信低
...
register_type("lowcredit_verb",r"(申请失败|没通过|申请不到|通不过|申请不了|审核不通过|信用烂了|征信烂了|征信差劲|逾期|没有资格|不通过|申请不下来|贷不下来)")
rules.add_rule("rule_zhongan_lowcredit","*{lowcredit_verb:lowcredit_verb}*")
# 脏话骂人
keyword_zhongan_dirty = "(我操|妈的|死|滚蛋|你妈|你妹|有病|他妈|鸡巴|神经病|妈个逼|脑子有问题|有毒|傻逼)&~(操作)"
register_type("dirty_verb",r"(我操|妈的|死|滚蛋|你妈|你妹|有病|他妈|鸡巴|神经病|妈个逼|脑子有问题|有毒|傻逼)")
rules.add_rule("rule_zhongan_dirty","*{dirty_verb:dirty_verb}*")
# 以后再说
...
register_type("talklater_verb",r"(看一下|再说|看看|再看|再联系|再会|稍后再打)")
rules.add_rule("rule_zhongan_talklater","*{talklater_verb:talklater_verb}*")
# 已经有了
keyword_zhongan_alreadyhave = "(申请了|申请过|不是有吗|申请完了|借过了|有你们的app|下载了)&~(没|不|一定要)"
register_type("material_verb",r"(申请了|申请过|不是有吗|申请完了|借过了|有你们的app)")
rules.add_rule("rule_zhongan_material","*{material_verb:material_verb}*")
# 厌恶
...
register_type("material_verb",r"(要|需要|有)")
register_type("material_noun",r"(材料|条件|抵押|手续)")
rules.add_rule("rule_zhongan_material","*{material_verb:material_verb}*{material_noun:material_noun}*")
# 问身份
...
register_type("company_verb",r"(什么)")
register_type("company_noun",r"(平台|公司|单位|APP)")
rules.add_rule("rule_zhongan_company","*{company_verb:company_verb}*{company_noun:company_noun}*")
# 未听清
...
register_type("notreceived_verb",r"(没有|没)")
register_type("notreceived_noun",r"(收到|看见|看到|接到|注意看)")
rules.add_rule("rule_zhongan_notreceived","*{notreceived_verb:notreceived_verb}*{notreceived_noun:notreceived_noun}*")
# 同意配合
keyword_zhongan_cooperate = "((加|发|留)&(微信|短信|链接|联系方式|一下|手机|号码|电话|给我))&~(什么|没|不|别|给我发|忙)"
register_type("cooperate_verb",r"(加|发|留)")
register_type("cooperate_noun",r"(微信|短信|链接|联系方式|手机|号码|电话)")
rules.add_rule("rule_zhongan_cooperate","*{cooperate_verb:cooperate_verb}*{cooperate_noun:cooperate_noun}*")
# 同行
...
register_type("flirt_verb",r"(亿|200万|50万|100万|一千万)")
rules.add_rule("rule_zhongan_flirt","*{flirt_verb:flirt_verb}*")
# 普通忙
...
register_type("commonbusy_verb",r"(现在忙|正忙|不方便|不太方便|在上班|在开会|没时间|没空|没时间|在忙|在睡觉|在午休|在午睡|在休息|信号不好|在电梯|信号太差|信号差|信号不好|信号不行|在吃饭|太忙了|挺忙|很忙|忙|现在有事|正在忙|这会有事|在工作|不得空)")
rules.add_rule("rule_zhongan_commonbusy","*{commonbusy_verb:commonbusy_verb}*")
# 年龄不合适
...
register_type("nomoney_verb",r"(利息|利率|利|手续费|厘|免息)")
rules.add_rule("rule_zhongan_nomoney ","*{nomoney_verb:nomoney_verb}*")
# 利息
...
register_type("interest_noun",r"(利息|利率|利|手续费|厘|免息)")
rules.add_rule("rule_zhongan_interest","*{interest_noun:interest_noun}*")
# 口碑差
keyword_zhongan_reputation = "(不靠谱|口碑差|骗人|诈骗|骗子|骗我|欺骗|骗来骗去|骗来|诈骗)&~(靠不)"
register_type("reputation_verb",r"(不靠谱|口碑差|骗人|诈骗|骗子|骗我|欺骗|骗来骗去|骗来|诈骗)")
rules.add_rule("rule_zhongan_reputation","*{reputation_verb:reputation_verb}*")
# 肯定
...
register_type("sure_deny",r"(不|没)")
register_type("sure_verb",r"(好|可以|需要|行|照|管|好的|办理|办一个|办一下)")
rules.add_rule("rule_zhongan_sure","*[^{sure_deny:sure_deny}]*{sure_verb:sure_verb}*")
# 开车忙
...
register_type("drive_verb",r"(在开车|在高速上|开着车|在车上|开车呢|开车|正开车)")
rules.add_rule("rule_zhongan_drive","*{drive_verb:drive_verb}*")
# 黑户
keyword_zhongan_black = "(黑户|黑名单)"
register_type("black_noun",r"(黑户|黑名单)")
rules.add_rule("rule_zhongan_black","*{black_noun:black_noun}*")
# 还款方式
keyword_zhongan_repay = "(还款方式|怎么还钱|自动还款|怎么还|手动还款|手动还|自动还|提前还)"
register_type("repay_verb",r"(还款方式|怎么还钱|自动还款|怎么还|手动还款|手动还|自动还)")
rules.add_rule("rule_zhongan_repay","*{repay_verb:repay_verb}*")
# 额度
...
register_type("unwanted_verb",r"(不需要|不考虑|不贷款|不感兴趣|没兴趣|不办|用不上|用不着|我有钱|不缺钱|不差钱|借到了|不用|不申请|不借钱|不要|不贷)")
rules.add_rule("rule_zhongan_unwanted","*{unwanted_verb:unwanted_verb}*")
# 不信任
...
register_type("noparty_verb",r"(打错了|找错人|打错号码|号码错误|不是本人|打错电话)")
rules.add_rule("rule_zhongan_noparty","*{noparty_verb:noparty_verb}*")
...
def Keyword_Sort_Score_RDG(query):
    # 关键词query对应每个关键词节点命中的情况
    keyword_intent_list = []
    keyword_ishit_list = []
    keyword_hitwordcnt_list = []
    keyword_unhitwordcnt_list = []
    keyword_intent_score = []
    for key,value in keyword_dict_rdg.items():
        matcher_bool,prefix_matcher_str_list,suffix_matcher_str_list,human_str_list = KeyWordMatcher(human_str=query,regex_str=value)
